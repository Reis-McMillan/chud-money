...
//! older frames carried integer cents (`price: 47`) and whole contracts
//! (`delta: -120`). Both are accepted and normalised to fixed-point integers:
//! prices in units of $0.0001, sizes in hundredths of a contract.
...
impl OrderbookMsg {
    ...
    fn levels(int: Option<&Vec<[i64; 2]>>, dec: Option<&Vec<[String; 2]>>) -> Vec<(i64, i64)> {
    ...
    fn delta_parts(&self) -> Option<(Side, i64, i64)> {
...
impl OrderBook {
    ...
    pub fn apply_msg(&mut self, msg: &OrderbookMsg, seq: u64) -> bool {
        if msg.is_snapshot() {
            let yes = OrderbookMsg::levels(msg.yes.as_ref(), msg.yes_dollars.as_ref());
            let no = OrderbookMsg::levels(msg.no.as_ref(), msg.no_dollars.as_ref());
